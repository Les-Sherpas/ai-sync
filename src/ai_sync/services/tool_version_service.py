"""Service for client version detection and lock checks."""
...
DEFAULT_VERSIONS_PATH = Path(__file__).resolve().parent.parent / ".client-versions.json"


class ToolVersionService:
    """Detect and validate local client versions."""

    def run_command_capture_output(self, cmd: list[str]) -> str:
        try:
    ...
    def detect_client_versions(self) -> dict[str, str]:
        ...
        for name, cmd in commands.items():
            ...
            if not cmd_path:
                continue
            output = self.run_command_capture_output([cmd_path, *cmd[1:]])
            match = VERSION_RE.search(output)
            if match:
                versions[name] = f"{match.group(1)}.{match.group(2)}.{match.group(3)}"
        return versions

    def get_default_versions_path(self) -> Path:
        return DEFAULT_VERSIONS_PATH

    def check_client_versions(self, versions_path: Path) -> tuple[bool, str]:
        ...
        current = self.detect_client_versions()
        if not current:
        ...
        for client, expected_version in expected.items():
            ...
            if not exp or not cur:
                return (
                    False,
                    f"Invalid version for {client} (expected {expected_version}, got {current[client]})",
                )
