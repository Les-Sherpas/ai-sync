...
class CodexClient(Client):
    ...
    def write_agent(self, slug: str, meta: dict, raw_content: str, prompt_src_path: Path) -> None:
        ...
        write_content_if_different(agent_dir / "config.toml", self._write_toml_config(agent_dir / "config.toml", config_data), backup=False)
    ...
    def sync_mcp(self, servers: dict, secrets: dict, for_client: Callable[[dict, str], bool]) -> None:
        ...
        write_content_if_different(config_path, self._write_toml_config(config_path, existing), backup=False)
    ...
    def sync_client_config(self, settings: dict) -> None:
        ...
        config_path = self.config_dir / "config.toml"
        existing = self._read_toml_config(config_path)
        write_content_if_different(config_path, self._write_toml_config(config_path, deep_merge(existing, updates)), backup=False)
