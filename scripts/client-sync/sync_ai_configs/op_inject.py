...
async def _load_runtime_env_async(env_template_path: Path) -> dict[str, str]:
    ...
    if token:
        ...
        raise RuntimeError(
            "1Password auth required. Set OP_SERVICE_ACCOUNT_TOKEN (service account) "
            "or OP_ACCOUNT (desktop app account name as shown in 1Password)."
        )

    client = await Client.authenticate(
        auth=auth,
        integration_name="sync-ai-configs",
...
def load_runtime_env_from_op(env_template_path: Path) -> dict[str, str]:
    if not env_template_path.exists():
        return {}
    return asyncio.run(_load_runtime_env_async(env_template_path))
